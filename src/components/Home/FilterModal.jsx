...
const FilterModal = ({ isOpen, onClose, filters, onFilterChange }) => {
  ...
  const CustomCheckbox = ({ id, value, checked, onChange, label }) => (
    <label htmlFor={id} className="flex items-center p-2 hover:bg-gray-50 rounded-md cursor-pointer">
      <input
        type="checkbox"
        id={id}
        value={value}
        ...
        className="sr-only"
      />
      <div className={`h-5 w-5 rounded flex items-center justify-center border ${checked ? 'bg-indigo-600 border-indigo-600' : 'border-gray-300'}`}>
  ...
  return (
    <AnimatePresence>
      ...
      >
        <motion.div
          className="bg-white w-full h-full sm:rounded-xl sm:shadow-xl sm:w-[95%] sm:max-w-xl sm:max-h-[90vh] overflow-hidden flex flex-col"
        ...
        >
            ...
            {showTabsMobile && (
              ...
              >
                {filterTabs.map(tab => (
                  <button
                    key={tab.id}
                    className={`flex items-center justify-between w-full py-3 px-4 text-left ${activeTab === tab.id ? 'bg-gray-100 text-indigo-600 font-medium' : 'text-gray-700'}`}
                    onClick={() => {
                      setActiveTab(tab.id);
          ...
          {/* Desktop Tabs */}
          <div className="hidden sm:flex border-b border-gray-200 overflow-x-auto scrollbar-hide">
            {filterTabs.map(tab => (
              <button
                key={tab.id}
                className={`py-3 px-4 relative whitespace-nowrap ${activeTab === tab.id ? 'text-indigo-600 font-medium' : 'text-gray-600'}`}
                onClick={() => setActiveTab(tab.id)}
              >
                {tab.label}
                {tab.count > 0 && (
                  <span className="absolute top-2 right-1 bg-indigo-600 text-white text-xs w-4 h-4 flex items-center justify-center rounded-full">
                    {tab.count}
                  </span>
                )}
                {activeTab === tab.id && (
          ...
          <div className="flex justify-between items-center p-4 sm:p-5 border-t border-gray-200 bg-gray-50">
            <button
