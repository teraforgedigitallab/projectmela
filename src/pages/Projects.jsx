...
import { FaFilter, FaSearch, FaTag, FaDollarSign, FaRegBookmark, FaBookmark } from 'react-icons/fa';
      ...
      <div className="flex flex-col md:flex-row md:items-center gap-4 flex-1 ">
        ...
        <div className="flex-1 min-w-0">
          <div className='absolute top-4 right-4 z-10'>
            ...
            >
              {isBookmarked ? <FaBookmark size={22} className="text-primary" /> : <FaRegBookmark size={22} />}
            </button>
          </div>
          ...
          <div className="flex flex-wrap gap-2 mb-4">
            ...
            <div className="flex items-center gap-1 relative group">
              ...
              <div className="absolute left-1/2 -translate-x-1/2 bottom-full mb-2 w-56 bg-black text-white text-xs rounded-sm px-3 py-2 opacity-0 group-hover:opacity-100 pointer-events-none transition-opacity z-30 text-center shadow-lg">
                Earn XP points by doing projects, More points you have bigger projects you can apply for
              </div>
              <MdStars className="text-warning" size={18} />
              <span className="font-medium text-xs text-primary">{points || 0} XP</span>
            </div>
            ...
            {tags.length > 0 ? (
              <>
                {tags.slice(0, 8).map((tag, idx) => (
                  <span
                    key={tag}
                    className="bg-primary/5 text-primary px-2 py-0.5 rounded text-xs font-medium flex gap-2"
                  >
                    <FaTag className="text-primary mt-0.5" />
                    {tag}
                  </span>
                ))}
                {tags.length > 8 && (
...
const Projects = () => {
  ...
  return (
    <section className="find-job section pb-12 bg-gray-100 min-h-screen">
      ...
      {(location.pathname==='/projects') && <ProfileBanner
